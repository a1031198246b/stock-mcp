...
A 股股票数据 MCP 服务，为 AI Agent 提供实时行情、K 线、基本面、资讯、自然语言选股等能力。

## ✨ 核心优势（差异化亮点）

| 特性 | 说明 |
|------|------|
| 🔌 **tqcenter 主源** | 本地通达信插件，实时行情 + **5档买卖盘**（其他项目少见） |
| 🛡️ **多源容错** | 内置熔断、限流、缓存，故障自动切换数据源 |
| 🔥 **自然语言选股** | 集成 iwencai，支持自然语言问股 |
| ✅ **91% 测试覆盖率** | 139 个测试，CI 完整，质量保证 |
| 📊 **清晰能力矩阵** | 多数据源能力一目了然，按需选用 |

## 功能概览

- **实时行情**：tqcenter / sina / akshare 多源自动故障转移
- **K 线**：1m/5m/15m/30m/1h/1d/1w/1M
- **基本面**：PE / PB / ROE / 市值 / 行业代码
- **资讯公告**：eastmoney / akshare
- **自然语言选股**：iwencai（需 cookie）
